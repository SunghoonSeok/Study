...
sub = read_csv('c:/data/test/solar/sample_submission2.csv', index_col=None, header=0)

# 마지막 하루의 데이터만 사용하겠습니다.
def preprocess_data(data):
    	temp = data.copy()
	return temp.iloc[-48:, :]

# test파일 하나로 합치기 + Day6만 골라내기
df_test = []
for i in range(81):
...
#Train and Validation split -> 이부분 발전 못시키려나
n = len(train)
...
sub.to_csv("/content/drive/MyDrive/data_solar/submission/submission_210114_quantile_ar_lstm_nn.csv",index=False)