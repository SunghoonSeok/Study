...
def hahaha(a, x_train, y_train, x_val, y_val, x_test):
    x = []
    for q in quantiles:
        filepath_cp = f'c:/data/test/solar/checkpoint/solar_checkpoint8_time{i}-{a}-{q}.hdf5'
...
train_data = preprocess_data(train_trans) # (52560,8)
...
print(x_test_data.shape) # (27216,8)
x_test_data = np.array(x_test_data)
x_test_data = x_test_data.reshape(81,7,48,8)
...
for i in range(48):
    ...
    x_test = x_test.reshape(567,8)
    x_test = scaler.transform(x_test)
    x_test = x_test.reshape(81,7,8)
...
sub.to_csv('c:/data/test/solar/sample_submission14_check.csv', index=False)        
