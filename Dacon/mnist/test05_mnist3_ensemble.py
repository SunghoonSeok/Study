...
x_train, x_test, y1_train, y1_test, y2_train, y2_test = train_test_split(x,y1,y2 test_size=0.2, shuffle=True)
scaler = MinMaxScaler()
scaler.fit(x_train)
...
# 2. 모델 구성
model = Sequential()
model.add(Conv2D(128, 2, padding='same', activation='relu', input_shape=(28,28,1)))
