...
def DaconModel():
    model = Sequential()
    model.add(Conv1D(512, 2, padding='same', input_shape=(1, 7),activation='relu'))
...
def hahaha(a, x_train, y_train, x_val, y_val, x_test):
    ...
    for q in quantiles:
        ...
        filepath = '/content/drive/My Drive/data/test/solar/checkpoint/solar_checkpoint_'+str(a)+'_'+str(q)+'.hdf5'
...
sub.to_csv('/content/drive/My Drive/data/test/solar/sample_submission7.csv', index=False)
...
def hahaha(a, x_train, y_train, x_val, y_val, x_test):
    x = []
    for q in quantiles:
        model = load_model('/content/drive/My Drive/data/test/solar/checkpoint/solar_checkpoint_'+str(a)+'_'+str(q)+'.hdf5', compile = False)
        # optimizer = Adam(lr=0.008)
        # model.compile(loss = lambda y_true,y_pred: quantile_loss(q,y_true,y_pred), optimizer = optimizer, metrics = [lambda y,y_pred: quantile_loss(q,y,y_pred)])
...
sub.loc[sub.id.str.contains("Day7"), "q_0.1":] = num_temp1
sub.loc[sub.id.str.contains("Day8"), "q_0.1":] = num_temp2
sub.to_csv('/content/drive/My Drive/data/test/solar/sample_submission7_check.csv', index=False)