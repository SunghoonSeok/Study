...
start = time.time()
models = XGBClassifier(n_jobs=8, eval_metric='mlogloss') # use_label_encoder=False)
search = [GridSearchCV, RandomizedSearchCV]


for k in search:
    model = k(models, parameters, cv=kfold)
    model.fit(x2_train, y_train, verbose=True, eval_set=[(x2_train,y_train),(x2_test,y_test)])
