...
class Dataset_tu(BaseDataset):
    ...
    def __init__(
        ...
        if mode == 'train':
            ...
            path = os.path.join(self.data_dir, 'task02_val\\')
        else:
        ...
        self.labels_fps = os.path.join(path, 'labels.json')
    ...
    #     for file_path in os.listdir(os.path.join(path,'new_train')):
    #         json_file = utils.load_json(os.path.join(path,'labels.json'))

    #         polygon_dict = json_file['annotations']
    #         polygon = []
