...
x2_train, x2_test, y2_train, y2_test, y3_train, y3_test = train_test_split(
    x2, y2, y3, shuffle=True, train_size=0.8
)
...
output3 = Dense(3)(output3)
 ...
 # 모델 선언
model = Model(inputs=[input1, input2], outputs=[output1, output2, output3]) # 2개 이상은 리스트로 묶는다
model.summary()

#3. 컴파일, 훈련
model.compile(loss='mse', optimizer='adam', metrics=['mae'])
model.fit([x1_train, x2_train], [y1_train, y2_train, y3_train],
           epochs=300, batch_size=8, validation_split=0.2, verbose=3)

#4. 평가, 예측
loss = model.evaluate([x1_test, x2_test], [y1_test, y2_test, y3_test], batch_size=8)
...
x2_pred = np.array([range(201, 211), range(511, 521), range(200,210)])

x1_pred = np.transpose(x1_pred)
x2_pred = np.transpose(x2_pred)


y1_predict, y2_predict, y3_predict = model.predict([x1_pred, x2_pred])
print("====================================")
print("y1_predict : \n", y1_predict)
print("====================================")
print("y2_predict : \n", y2_predict)
