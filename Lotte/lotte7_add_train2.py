...
train_generator = idg.flow(x_train,y_train,batch_size=64, seed = 120)
# seed => random_state
valid_generator = idg2.flow(x_valid,y_valid)
# test_generator = idg2.flow(x_pred)

mc = ModelCheckpoint('../data/modelcheckpoint/lotte_projcet1.h5',save_best_only=True, verbose=1)
...
# predict
model.load_weights('../data/modelcheckpoint/lotte_projcet1_3.h5')
result = model.predict(x_pred,verbose=True)


sub = pd.read_csv('../data/lotte2/sample.csv')
sub['prediction'] = np.argmax(result,axis = 1)
sub.to_csv('../data/lotte2/answer (6).csv',index=False)